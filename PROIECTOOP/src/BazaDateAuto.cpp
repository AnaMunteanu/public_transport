...
vector<Autobuz> BazaDateAutobuze::filterByTime(Statie pornire,Statie oprire, Ora O){
    //functie care returneaza lista autobuzelor care pleaca cel mai repede din statia pornire spre statia oprire dupa ora O
    //input: Statie pornire, Statia oprire, Ora O
    //output:vector de autobuze
    vector<Autobuz> x;
    x.clear();
    vector<Ora> L;
    L.clear();
...
Ora BazaDateAutobuze::Fulger(Autobuz A,Statie S, Ora O){
	/*Descr:returneaza prima ora mai mare ca ora O la care autobuzul A trece prin statia S
	 * In:Autobuz A, statia S, ora O
	 * Out:Ora
	 */
...
bool BazaDateAutobuze::esteAutobuz(int nr){
	/*Descr:verifica daca autouzul cu numarul "nr" este in vectorul de autobuze
	 * In:nr
	 * OutT/F
	 */
    for(int i=0;i<V.size();i++)
...
bool BazaDateAutobuze::esteStatie(string nume){
	/*Descr:verifica daca statia cu numele "nume" este pe traseul vreunui autobuz din vectorul de autobuze
		 * In:nume
		 * OutT/F
		 */
    for(int i=0;i<V.size();i++)
