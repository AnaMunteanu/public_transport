...
void UI::calatorie(int pos)
{
   ...
   // cout<<traseu.toString()<<endl;
    ...
    if(decizie.find("Acum")==0||decizie.find("acum")==0)
    {
        ...
        std::cout<<"Introduceti ora dorita pentru calatorie:"<<'\n';
        std::cout<<"Ora: ";
        O=citireora();
        }
    Statie s;
    ...
    if(traseu.getall().size()!=0)
    {
        ...
        std::cout<<"Pentru a ajunge din statia "<<pornire<<" in statia "<<sosire<<" puteti lua autobuzele: "<<'\n';
...
void UI::client_logat()
{
    ...
    if(optiune==5)
    {
    	...
    	cout<<"Username sau parola incorecta, va rugam sa incercati din nou"<<endl;
        client_logat();
    }
}
void UI::sign_up()
