...
bool Autobuz::isStatie(Statie S){
	 //functie care verifica daca statia S apartine vectorului de statii din orarul autobuzului
		//In:Statia S
		//Out:: T/F
    return orar.isStatie(S);
}
bool Autobuz::isStatie(string nume){
	//functie care verifica daca statia cu numele "nume" apartine vectorului de statii din orarul autobuzului
		//In:numele statiei
		//Out:: T/F
    return orar.isStatie(nume);
}
int Autobuz::posStart(Statie pornire, Statie oprire){
	/*Desc:returneaza pozitia pe care se afla statia start inaintea statiei stop
		 * In:statiile start si stop
		 * Out:pozitia din vectorul de statii
		 */
